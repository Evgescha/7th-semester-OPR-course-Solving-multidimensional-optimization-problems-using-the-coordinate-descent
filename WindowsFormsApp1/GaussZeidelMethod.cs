...
namespace WindowsFormsApp1
{
    ...
    class GaussZeidelMethod
    {
        ...
        internal GoldenRatioMethod GoldenRatioMethod
        {
            get => default(GoldenRatioMethod);
            set
            {
            }
        }
        ...
        public static Point2D FindMinimum(Func<double, double, double> f, Point2D x0, Point2D h, double eps0)
        {
            ...
            Logger.Write("===> Минимизация методом покоординатного спуска <==");
            ...
            do
            {
                ...
                if (form.radioButton1.Checked && OptimizationUtils.EuclidNorm(h) <= eps0) break;
                if (form.radioButton2.Checked && OptimizationUtils.EuclidNorm(h) >= eps0) break;
                //Уменьшаем шаг
                h.X = z * h.X;
                h.Y = z * h.Y;
            ...
            Logger.Write("Минимум(Максимум) найден: {0:" + format + "}", x);
